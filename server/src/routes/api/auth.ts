...
export const registerAuthRoutes = (parent: Router) => {
	...
	router.get('/me',
		...
		// todo: figure out how to clear the cache on session update
		// assignCacheControlMiddleware(new Duration({ minutes: 1 }), false /*isPublic*/),
		async ctx => {
			const serverUser = await getUserOrThrowAsync(ctx);

			const clientUser: IClientUserDTO = {
				id:          serverUser.id,
				displayName: serverUser.displayName,
				provider:    serverUser.provider,
				createdAt:   serverUser.createdAt.getTime(),
				role:        serverUser.role,
			};

			if (serverUser.settings != null) {
				clientUser.settings = {
					...serverUser.settings,
					lastUpdate: serverUser.settings.lastUpdate.getTime()
				};
			}
