...
import { ICafe, ICafeConfig, ICafeStation, IMenuItem, IMenuItemTag, } from '../../models/cafe.js';
...
export abstract class CafeStorageClient {
	...
	private static async _performMultiQuerySearch(queries: Array<SearchTypes.ISearchQuery>, shouldUseExactMatch: boolean): Promise<Map<SearchTypes.SearchEntityType, Map<string, ISearchResult>>> {
		...
		const normalizedQueries: Array<SearchTypes.ISearchQuery> = queries.map(({ text, type }) => {
			return {
	...
	public static async search(query: string): Promise<Map<SearchTypes.SearchEntityType, Map<string, ISearchResult>>> {
		return CafeStorageClient._performMultiQuerySearch([{ text: query }], false /*shouldUseExactMatch*/);
	}

	public static async searchFavorites(queries: Array<SearchTypes.ISearchQuery>): Promise<Map<SearchTypes.SearchEntityType, Map<string, ISearchResult>>> {
		return CafeStorageClient._performMultiQuerySearch(queries, true /*shouldUseExactMatch*/);
	}
