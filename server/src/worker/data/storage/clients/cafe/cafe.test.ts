...
test('two concurrent retrieveCafesAsync() calls hit Prisma exactly once', async (testContext) => {
    ...
    const prismaClient = await usePrismaClient(async (prisma) => prisma);
    ...
    assert.equal(firstResult.size, SEEDED_CAFE_IDS.length);
    assert.equal(secondResult.size, SEEDED_CAFE_IDS.length);
    // Both calls receive the same underlying map instance.
    assert.equal(firstResult, secondResult);
    for (const id of SEEDED_CAFE_IDS) {
        assert.ok(firstResult.has(id), `expected cafe ${id} in cache`);
    }
});

test('cache populated exactly once even under heavier contention', async (testContext) => {
    for (let i = 0; i < SEEDED_CAFE_IDS.length; i++) {
        await seedCafe(SEEDED_CAFE_IDS[i]!, `Cafe ${i}`);
    }
...
test('previously-cached cafes are NOT returned after resetCache() drops them from the DB', async () => {
    ...
    await usePrismaWrite(prisma => prisma.cafe.delete({ where: { id: 'test-cafe-a' } }));
