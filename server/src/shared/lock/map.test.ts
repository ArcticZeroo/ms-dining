...
test('concurrent update() calls for the same key serialize (no interleaving)', async () => {
    ...
    const final = await map.update('k', value => value);
...
test('update() that throws does not poison the lock', async () => {
    ...
    const result = await map.update('k', (value) => {
        observedValue = value;
        return (value ?? 0) + 1;
    });
...
test('delete() during contention waits for in-flight update to finish', async () => {
    ...
    const updatePromise = map.update('k', async (value) => {
        await wait(40);
        updateFinished = true;
        return value;
    });
...
test('LockedExpiringMap: updateWithoutRefresh() keeps original expiration', async () => {
    ...
    try {
        ...
        await wait(100);
        await map.updateWithoutRefresh('k', (value) => (value ?? 0) + 1);

        // Value should be updated
        let observed: number | undefined;
        await map.peek('k', (value) => {
            observed = value; 
        });
...
test('LockedExpiringMap: update() resets expiration', async () => {
    ...
    try {
        ...
        await wait(100);
        await map.update('k', (value) => (value ?? 0) + 1);
