/**
 * Fixture generator: produces per-cafe fixture data for ALL cafes from a pool
 * of station/item templates using seeded random for deterministic output.
 *
 * Run with: npx tsx src/test-server/generate-fixtures.ts
 *
 * Key invariants:
 *  - Station and item IDs are globally unique per-cafe (namespaced with cafe ID)
 *  - Items may appear in multiple categories within the same station (intentional)
 *  - Each cafe gets a deterministically different set of stations/items
 */
...
// ── All cafe IDs (mirrors constants/cafes.ts) ──────────────────────

const ALL_CAFE_IDS = [
    'foodhall6', 'cafe16', 'cafe25', 'cafe31', 'cafe34', 'cafe36', 'cafe37',
    'cafe40-41', 'cafe43', 'cafe50', 'cafe83', 'cafe86', 'cafe99', 'cafe109',
    'cafe121', 'caferedwest', 'cafertc5', 'cafestudioh', 'one-esterra', 'cafemille',
    'chandys', 'justpoke', 'himalaya', 'hometown', 's-wich', 'typhoon', 'mayas',
    'foodhall4', 'fh4jacksbbq', 'fh4joesburger', 'fh4justpoke', 'fh4mila',
    'fh4papparepas', 'fh4boonboona', 'foodhall9', 'fh9bigchicken',
    'dote', 'l-experience', 'pinkabella', 'bobae',
    'boardwalk', 'craft75', 'in-gredients',
    'studioa', 'cafe92', 'studiod', 'dote-rts', 'b3espresso', 'b7espresso',
];
...
function generateForCafe(cafeId: string): GeneratedFixtures {
...
for (const cafeId of ALL_CAFE_IDS) {
