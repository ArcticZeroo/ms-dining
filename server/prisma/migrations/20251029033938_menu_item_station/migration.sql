/*
  ...
  - Added the required column `stationId` to the `MenuItem` table without a default value. This is not possible if the table is not empty.
...
INSERT INTO "new_MenuItem" ("cafeId", "calories", "description", "externalLastUpdateTime", "externalReceiptText", "groupId", "id", "imageUrl", "maxCalories", "name", "normalizedName", "price", "tags", "stationId")
SELECT MenuItem.cafeId, MenuItem.calories, MenuItem.description, MenuItem.externalLastUpdateTime, MenuItem.externalReceiptText, MenuItem.groupId, MenuItem.id, MenuItem.imageUrl, MenuItem.maxCalories, MenuItem.name, MenuItem.normalizedName, MenuItem.price, MenuItem.tags, Station.id
FROM "MenuItem"
