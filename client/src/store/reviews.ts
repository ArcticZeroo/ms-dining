...
class ReviewStore {
    readonly #reviewsByMenuItemId = new Map<string, LazyResource<IReviewSummary>>();
    readonly #reviewsByStationId = new Map<string, LazyResource<IReviewSummary>>();
    ...
    #getOrCreateResource(menuItemId: string): LazyResource<IReviewSummary> {
    ...
    #getOrCreateStationResource(stationId: string): LazyResource<IReviewSummary> {
        let resource = this.#reviewsByStationId.get(stationId);
        if (!resource) {
            resource = new LazyResource(() => DiningClient.retrieveReviewsForStation(stationId));
            this.#reviewsByStationId.set(stationId, resource);
        }
        return resource;
    }
    ...
    #evictStaleEntries() {
        ...
        for (const [stationId, resource] of this.#reviewsByStationId) {
            if (now - resource.lastAccessed > EVICTION_MAX_IDLE_MS) {
                this.#reviewsByStationId.delete(stationId);
            }
        }
    }
    ...
    getStationReviews(stationId: string) {
        return this.#getOrCreateStationResource(stationId).get();
    }
    ...
    async createStationReview(stationId: string, request: ICreateReviewRequest, context: ICreateReviewContext): Promise<string> {
        const reviewId = await DiningClient.createStationReview(stationId, request);
        ...
        const resource = this.#reviewsByStationId.get(stationId);
        await resource?.updateExisting((current) => {
            const counts = { ...current.counts };
            counts[request.rating] = (counts[request.rating] ?? 0) + 1;
            const totalCount = current.totalCount + 1;
        ...
        await this.#updateRecentReviews((recent) => {
            return [review, ...recent].slice(0, RECENT_REVIEWS_COUNT);
        });

        if (!request.anonymous) {
            await this.#updateMyReviews((mine) => {
                const filtered = mine.filter((existing) => existing.stationId !== stationId);
                return [review, ...filtered];
            });
        }
    ...
    async deleteStationReview(reviewId: string, stationId: string): Promise<void> {
        await DiningClient.deleteReview(reviewId);
    ...
    async updateStationReview(reviewId: string, stationId: string, request: IUpdateReviewRequest): Promise<void> {
        await DiningClient.updateReview(reviewId, request);
